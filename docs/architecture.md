...
Plan downgrades are handled in `PaddleWebhookService`. When `subscription.canceled` / `paused` / `past_due` arrives, the handlers only flip `Subscription::status` — they don't delete employees, managers, or sub-QRs. The next `canAddEmployee()` / `canPromoteToManager()` call simply returns `false` (because `getActivePlan()` now returns `Free`), so existing records remain visible until the owner upgrades or removes them. Workspace deletion is the inverse: `WorkspaceService::delete()` proactively cancels the Paddle subscription via API before soft-deleting locally, swallowing API failures so a Paddle outage can't block the local delete. It cancels anything that isn't already `canceled` — deliberately *not* gated on `isActive()`, which is `active|trialing` only, because a `past_due` or `paused` subscription is still a live billing agreement at Paddle.

**Cancellation is terminal.** Once `Subscription.canceledAt` is set, the `updated` / `past_due` / `paused` / `resumed` handlers ignore the event rather than writing a new status. Paddle has no un-cancel (a resume applies to a paused subscription, never a cancelled one), so a status event arriving afterwards is dunning that was already in flight, and acting on it resurrects a subscription the customer — or a workspace deletion — already ended. The guard keys on `canceledAt` rather than the status so that rows which already drifted stop drifting further. A genuine re-subscribe is unaffected, because Paddle mints a new subscription id: that arrives as `subscription.created`, takes over the workspace's single subscription row, and clears the cancellation. For the same reason `cancelSubscription()` never overwrites an existing `canceledAt` — churn reporting counts by that date, and moving it would re-churn a customer who left months ago.
...
- **`bangkeut/tap-core`** — framework-free verification of a credential presented at a terminal. Two credential kinds share one wire envelope: *device assertions* (the holder's device signs the terminal's nonce — unforgeable and unreplayable, but needs an app) and *issued passes* (the issuer signs once at issuance — verifiable offline with no app, but a bearer token, so anti-passback is the control). The byte layout is pinned in [packages/tap-core/SPEC.md](../packages/tap-core/SPEC.md) and asserted by `AssertionCodecTest`, because device and issuer SDKs in other languages check themselves against it.
  ...
  The package covers both directions for passes: `TapVerifier` at the door, and `Issuance\PassIssuer` at the point of sale, which signs a pass and returns it as raw bytes (NFC) and base64url (QR code, wallet payload, JSON). Issuance is where mistakes have to be caught — the validity window is inside the signature, so a pass minted with a wrapped uint32 timestamp or an inverted window can't be corrected at the gate; `PassIssuanceFailed` refuses those up front. There is no device-side signer in PHP by design: a holder's private key belongs in Android Keystore or a Secure Enclave.

  Withdrawing a pass is the asymmetric case. A device key is revoked by dropping it from the host's `CredentialStore`, so the credential simply stops matching; a pass carries its own authority in a signature the issuer can no longer reach, so revocation has to be out of band — the terminal is told, via an optional `Revocation\RevocationStore`. The verifier consults it after the signature verifies (a forger must not be able to use a door as an oracle for which pass ids exist) and before the anti-passback cooldown is consumed. It fails closed: a store that throws refuses the tap, because a door that can't tell whether a ticket was refunded shouldn't wave people through. Offline, a door enforces the list it last synced — a pass withdrawn mid-outage is admitted until the next sync, which is a property of being offline rather than a defect.
