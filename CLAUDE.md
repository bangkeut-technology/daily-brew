...
**Late/early.** Computed at check-in/out vs assigned Shift + grace minutes. No shift → always false. **`attendanceTracking=none` employees never get flagged regardless of shift** — `AttendanceFlagCalculator` (shared by `CheckinService` and `AttendanceService::override`) short-circuits when `!$employee->isAttendanceTracked()`. All comparisons in workspace TZ. **Per-day rules = complete schedule** (Espresso, when `Shift::hasAnyTimeRules()`): the resolver returns the matching rule's start/end, or `null` if today has no rule — late/leftEarly flags are then suppressed (the day is off-shift, not "fall back to the default start/end"). Free-tier shifts and Espresso shifts without per-day rules keep the legacy "default times apply every day" behavior.
...
**Platform admin (`/admin/*`).** Gated by `ROLE_SUPER_ADMIN`. Bootstrap: idempotent CLI `dailybrew:admin:promote-user <email>` (only path without existing super-admin); then promote/demote in UI (`POST /admin/users/{publicId}/promote|demote`), self-demote rejected (400). `UserDTO.isSuperAdmin` exposed. `AdminAuditService::record()` is wrap-and-log — failures swallowed so audit can't roll back the already-flushed action. Endpoints under `/api/v1/{locale}/admin`: dashboard, workspaces (with restore), users, subscriptions, churn, audit-log. `/admin/*` redirects to `/console/dashboard` if not super-admin.
...
**Staging subdomain.** `next.dailybrew.work` is a real public Next.js mirror that shares prod DB + Symfony API; one `dailybrew-next.service` serves both `server_name`s. The amber "Staging mirror — actions affect production data" banner (`frontend/src/components/StagingBanner.tsx`) is a client component that fires when `window.location.hostname` starts with `next.` or `staging.` — deliberately client-side because a server-side `headers()` read would force every marketing route out of static generation (verified: `next build` keeps every route at `○ Static` except `/checkin/[qrToken]`). Cookies stay scoped to the subdomain, so signing in on `next.*` creates a fresh session separate from your prod login. **OAuth providers must whitelist `https://next.dailybrew.work/oauth/{google,apple}/callback`** before sign-in via Google/Apple works on the subdomain — otherwise `redirect_uri_mismatch`. The console served on `next.*` is the in-progress Next port (~13% feature parity vs SPA today); use the optional `auth_basic` block in the nginx example to gate access while console parity catches up.

**Frontend parity status (as of 2026-06-09, v1.106.0).** Marketing + auth + checkin + blog index are at or near parity; `/blog` has three SPA-only posts not yet ported (`the-buddy-punching-tax`, `live-checkin-alerts-on-telegram`, `the-new-device-alert`); landing/pricing has stale `$14.99/$149` Espresso copy (real prices are `$19.99/$199` per PR #236) in both the hero `PricingSection` and the `en/fr/km` JSON locales. Console is the big gap: dashboard, employees, settings, profile, employee detail, employee new form, shifts per-day rules editor, attendance gantt+summary+log views, qr-codes sub-QR detail page are all missing or skeletal in Next; see PR #297 description for the full triage. Don't rely on Next console for prod use until that backlog clears.
