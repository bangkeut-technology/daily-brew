...
// Marketing pages are static and provider-free (no QueryClient / AuthProvider),
// so the legacy feature-flag (useFeatures) and auth (useAuthenticationState)
// gating can't run here. A public visitor is treated as unauthenticated — only
// requireAuth:false step links (sign up / sign in / NFC guide) are shown — and
// all steps are visible since the NFC guide is already a public, indexable page.
...
export function PlaybookSection({ playbook }: Props) {
  ...
  const visibleSteps = playbook.steps;
  return (
    <section
    ...
    >
      ...
      <header className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-[14px] text-text-secondary mt-1">{playbook.subtitle}</p>
        </div>
      </header>

      <ol className="space-y-4">
        {visibleSteps.map((step, stepIndex) => {
          const showLink = step.link && step.link.requireAuth === false;
          return (
            <motion.li
              key={step.title}
            ...
            >
              ...
              <div className="flex-1 pt-0.5">
                ...
                <p className="text-[14px] text-text-secondary leading-relaxed mt-1">{step.desc}</p>
                {showLink && step.link && <StepLink link={step.link} accent={playbook.accent} />}
              </div>
            </motion.li>
          );
        })}
