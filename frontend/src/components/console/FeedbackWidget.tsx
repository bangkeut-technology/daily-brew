...
/**
 * Outer guard: only surface the launcher when feedback is actually wired up on
 * the server. The legacy SPA reads that from `window.__DAILYBREW__`, which
 * Symfony injects per request; Next serves its own HTML, so it asks
 * `GET /support/config`. Without the guard, `POST /support/feedback` 502s — and
 * the user only finds out after typing the whole report.
 */
export function FeedbackWidget() {
  const { data } = useSupportConfig();
  if (!data?.feedbackEnabled) return null;
