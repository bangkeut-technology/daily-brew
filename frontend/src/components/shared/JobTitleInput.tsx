...
export function JobTitleInput({
  ...
  // Reset the highlight when the filter set changes. Done during render (the
  // sanctioned "adjust state when a derived value changes" pattern) rather than
  // in an effect, which would cost an extra render pass on every keystroke.
  const [lastFilteredLength, setLastFilteredLength] = useState(filtered.length);
  if (lastFilteredLength !== filtered.length) {
