...
export function QrScanner({ onDecode, unavailableText }: QrScannerProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      // The capability probe lives inside the async body so nothing sets state
      // synchronously in the effect: `navigator.mediaDevices` can't be read
      // during SSR, so it can't move into a state initializer without causing
