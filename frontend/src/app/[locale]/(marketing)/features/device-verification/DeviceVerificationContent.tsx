...
// Stable keys for the 4 "how it works" cards.
const howKeys = ['uniqueId', 'lockDevice', 'oneDevice', 'auditTrail'] as const;
const howIcons = [<Fingerprint key="f" size={20} />, <Smartphone key="s" size={20} />, <UserX key="u" size={20} />, <ClipboardList key="c" size={20} />];

const benefitKeys = ['eliminate', 'onePhone', 'audit', 'frictionless'] as const;
const benefitIcons = [<UserX key="u" size={20} />, <Users key="g" size={20} />, <ClipboardList key="c" size={20} />, <CheckCircle key="ch" size={20} />];

const setupKeys = ['openSettings', 'enable', 'save'] as const;
const setupIcons = [<Settings key="set" size={20} />, <ToggleRight key="t" size={20} />, <Save key="s" size={20} />];

const experienceKeys = ['normal', 'wrongDevice', 'alreadyUsed'] as const;
            ...
            <div className="space-y-5">
              {experienceKeys.map((key, i) => (
                <div key={key} className="flex items-start gap-3">
                  ...
                  <p className="text-[15px] text-text-secondary leading-relaxed">
                    <span className="font-medium text-text-primary">{t(`${ns}.experience.items.${key}.label`)}</span>{' '}
                    {t(`${ns}.experience.items.${key}.desc`)}
                  </p>
                </div>
              ))}
            </div>
