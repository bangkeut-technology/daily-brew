...
const venues: { icon: React.ReactNode; key: string; accent: string }[] = [
...
// The big numerals are kept as raw strings — '3', '< 5', '0' read the same in
// every locale we ship. Only the label + sub get translated.
const highlights: { value: string; key: string }[] = [
        ...
        {venues.map((v, i) => (
          ...
          >
            ...
            <span className="text-[14px] font-medium text-text-primary text-center">
              {t(`homepage.builtFor.venues.${v.key}`)}
            </span>
          </motion.div>
        ))}
      </div>
          ...
          {highlights.map((h, i) => (
            ...
            >
              ...
              <p className="text-[15px] font-semibold text-text-primary mb-0.5">
                {t(`homepage.builtFor.highlights.${h.key}.label`)}
              </p>
              <p className="text-[13.5px] text-text-tertiary">
                {t(`homepage.builtFor.highlights.${h.key}.sub`)}
              </p>
            </motion.div>
          ))}
        </div>
