...
const benefits: { icon: React.ReactNode; key: string }[] = [
  { icon: <ArrowRightLeft size={18} strokeWidth={1.8} />, key: 'sync' },
  { icon: <FileText size={18} strokeWidth={1.8} />,       key: 'link' },
  { icon: <Clock size={18} strokeWidth={1.8} />,          key: 'period' },
];
    ...
    <section className="py-24 px-6 md:px-8 max-w-5xl mx-auto">
      ...
      >
        <p className="text-[13px] uppercase tracking-[2px] font-medium text-amber mb-3">
          {t('homepage.integration.eyebrow')}
        </p>
        <h3 className="text-[30px] md:text-[36px] font-semibold text-text-primary font-serif leading-tight">
          {/* "Works with <BasilBookBrand />" — the brand stays a wordmark, the
              surrounding prose is per-locale. */}
          <Trans
            i18nKey="homepage.integration.titleParts"
            components={{ brand: <BasilBookBrand /> }}
          />
        </h3>
        <p className="text-[16px] text-text-secondary mt-3 max-w-lg mx-auto">
          {t('homepage.integration.subtitle')}
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="p-8 md:p-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[13px] text-text-tertiary">{t('homepage.integration.brand.attendance')}</p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-[13px] text-text-tertiary">{t('homepage.integration.brand.accounting')}</p>
              </div>
            </a>
          </div>
            ...
            {benefits.map((b, i) => (
              ...
              >
                ...
                <h4 className="text-[15px] font-semibold text-text-primary leading-snug">
                  {t(`homepage.integration.benefits.${b.key}.title`)}
                </h4>
                <p className="text-[14px] text-text-secondary leading-relaxed">
                  {t(`homepage.integration.benefits.${b.key}.desc`)}
                </p>
              </motion.div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 px-5 py-4 rounded-xl bg-cream-3/30 dark:bg-cream-3/10 border border-cream-3/40">
            <div className="flex-1">
              <p className="text-[14.5px] text-text-secondary leading-relaxed">
                <span className="font-medium text-text-primary">{t('homepage.integration.howItWorks.label')}</span>{' '}
                {/* "Generate an API token in DailyBrew settings. <brand /> uses
                    it to pull attendance data for employees with matching
                    usernames — check-in times, late flags, and shift info." */}
                <Trans
                  i18nKey="homepage.integration.howItWorks.body"
                  components={{ brand: <BasilBookBrand className="text-[14.5px]" /> }}
                />
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-1 rounded-full bg-amber/10 text-amber border border-amber/15">
                <Crown size={11} />
                {t('homepage.integration.espressoPill')}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
            ...
            >
              {t('homepage.integration.ctaPrimary')}
            ...
            >
              {/* "Learn about <brand />" */}
              <Trans
                i18nKey="homepage.integration.ctaSecondaryParts"
